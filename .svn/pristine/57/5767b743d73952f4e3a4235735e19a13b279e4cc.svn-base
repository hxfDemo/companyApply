...
public class ApiController {
    ...
    @Autowired
    private TKnowledgeTestService tKnowledgeTestService;

    @Autowired
    private IAnswerInfoService answerInfoService;
    ...
    public Result login(String username, String password) {
        ...
        password = ToolUtil.decodepwd(password);
    ...
    //datetime,模拟考试时间
    //datetime2 正式考试时间
    @PostMapping("/resetdlsj")
    public Result resetdate(String datetime,String datetime2) {
       ...
       Users user= new Users();
        ...
        try {
           user.setId(userId);
           if (datetime!=null){
               user.setSpare1(datetime);
           }else if(datetime2!=null){
               user.setSpare2(datetime2);
           }
          iUsersService.updateById(user);
           Map<String, Object> getuserkszt = iUsersService.getuserkszt(userId + "");
           if (getuserkszt!=null){
               map.put("data",getuserkszt);
               return Result.ok(map);
           }else {
               return Result.error(901,"获取考试状态为空");
           }
       }catch (Exception e){
           e.printStackTrace();
           return Result.error(901,"维护中。。。");
       }
    }
    /**
     * 获取注册单位审核信息
    ...
    public Result usersDelete1(@NotNull(message = "ID[id]不能为空") Long id) {
        try {
            Users byId = iUsersService.getById(id);
            if (byId == null) {
                return Result.fail(203, "获取单位考生信息失败！");
            }
            int i = iUsersService.getBaseMapper().deleteById(id);
            if (i>0){
                return Result.ok("删除用户信息成功！");
            }else {
                return   Result.error(203,"删除用户信息失败！");
            }
        }catch (Exception e){
            e.printStackTrace();
            return   Result.error(203,"系统维护中！");
        }
...
//           }
     ...
     *  id 用户id
     */
    @PostMapping("/getuserkszt")//获取考生状态
    public Result getuserkszt(@NotNull(message = "ID[id]不能为空") Long id) {
        Users byId = iUsersService.getById(id);
        if (byId == null) {
            return Result.error(203, "获取用户信息失败！");
        }
        try {
            Map<String, Object> getuserkszt = iUsersService.getuserkszt(id + "");
            if (null == getuserkszt) {
                return Result.error(203, "获取用户考试状态信息失败！");
            }
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("info", getuserkszt);
    ...
    /*
     * Integer mukszt,模拟考试状态
     * Integer zzkszt, 正式考试状态 状态值 0 未考试 1 开启考试
     * String mnkskm, 模拟测试id
     * String zzkskm,正式测试id
     * Integer companyid 单位id
     * type 考试类型 0 模拟，1 正式
     * */
    @PostMapping("/updateCompanykszt")//修改考生考试状态
    public Result updateCompanykszt(Integer mukszt,String mnkskm,String zzkskm, Integer zzkszt,Integer companyid,Integer type,Integer qid) {
        ...
        if (role.isEmpty()) {
            return Result.error(203, "获取权限失败！");
        }
        if (!"user".equals(role.trim())) {
            return Result.error(203, "无此权限！");
        }
        Company company = new Company();
        try {

            String currentTime = ToolUtil.getCurrentTime(null, new Date());
            TKnowledgeTest  tKnowledgeTest = tKnowledgeTestMapper.selectById(qid);
            String mntime = tKnowledgeTest.getSpare2();
            String zstime = tKnowledgeTest.getSpare3();
            if (mntime!=null){
                if (ToolUtil.getdiffDate(mntime)<20){
                    return Result.error(203, "开启模拟考试时间未过20分钟不能开启考试！");
                }
            }
            if (zstime!=null){
                if (ToolUtil.getdiffDate(zstime)<20){
                    return Result.error(203, "开启正式考试时间未过20分钟不能开启考试！");
                }
            }
            if (type.equals(0)){
                tKnowledgeTest.setId(Long.valueOf(qid));
                tKnowledgeTest.setMnks(mukszt);
                tKnowledgeTest.setMnkskm(Integer.parseInt(mnkskm));
                tKnowledgeTest.setSpare2(currentTime);//模拟开始时间
                company.setId(Long.valueOf(companyid));
                company.setMnks(mukszt);
                company.setSpare2(currentTime);//模拟开始时间
                company.setMnkskm(Integer.parseInt(mnkskm));
                if (1==mukszt) {
                    int testCount = tKnowledgeTest.getTestCount();
                    if (1<=testCount) {
                        Integer i = testCount - 1;
                        tKnowledgeTest.setTestCount(i);
//                int i1 = tKnowledgeTestMapper.updateById(tKnowledgeTest);
                    }
                }
            }else {
                tKnowledgeTest.setId(Long.valueOf(qid));
                tKnowledgeTest.setZsks(zzkszt);
                tKnowledgeTest.setZskskm(Integer.parseInt(zzkskm));
                tKnowledgeTest.setSpare3(currentTime);//正式开始时间
                company.setId(Long.valueOf(companyid));
                company.setZsks(zzkszt);
                company.setSpare3(currentTime);//正式开始时间
                company.setZskskm(Integer.parseInt(zzkskm));
            }
            boolean b1 = iCompanyService.updateById(company);
            if (b1==false){
                return Result.error(203, "修改公司信息失败！");
            }
            boolean b = tKnowledgeTestService.updateById(tKnowledgeTest);
            if (b) {
                return Result.ok("修改考试状态成功！");
            } else {
                return Result.error(203, "开启考试失败！");
            }
        } catch (Exception e) {
            e.printStackTrace();
            return Result.error(901, "系统维护中。。。");
    ...
    @PostMapping("/wjdcScore")//获取调查问卷信息及成绩
    public Result wjdcScore(String cname, Integer current, Integer size) {
        try {
            Long userId = TokenUtil.getUserId();
            String role = iUsersService.getRole(userId);
            if (role.isEmpty()) {
            ...
            PageHelper.startPage(current != null ? current : 1, size != null ? size : 10);
            List<Map<String, Object>> stringObjectMap = answerInfoService.wjdcScore(cname);
            PageInfo<Map<String, Object>> page = new PageInfo<Map<String, Object>>(stringObjectMap);
            if (stringObjectMap == null) {
                return Result.error(203, "未查到成绩信息！");
            }
            Map<String, Object> map = new HashMap<>();
